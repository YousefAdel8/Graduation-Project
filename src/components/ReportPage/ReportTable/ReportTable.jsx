			...
			>
				<Input
					...
					placeholder={
						En
							? `Search ${dataIndex}`
							: `بحث عن ${fieldTranslations[dataIndex]}`
					}
	...
	const updateStatus = () => {
		if (selectedRowId) {
			setTableData((prevData) =>
				prevData.map((row) =>
					row.key === selectedRowId ? { ...row, status: selectedStatus } : row
				)
			);
			...
			{showUpdateDiv && (
				...
				>
					...
					>
						...
						<div className="mb-4">
							<label className="block text-sm font-medium mb-2 ms-2">
								{En ? "Select Status " : " حدد الحالة "} :{" "}
							</label>
							<Select
								className=""
